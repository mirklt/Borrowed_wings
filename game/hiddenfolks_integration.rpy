# hf - HiddenFolks integration for main game
# This file is a copy of HiddenFolks.rpy from the HiddenFolks_test project,
# allowing the mini-game to be used inside the main visual novel.

# используется в паре с модулем 7dots.rpy (нативная часть здесь не нужна)
...
# ВНУТРЕННИЕ ПЕРЕМЕННЫЕ
    ...
    def hf_start(effect=None):
        global hf_game_mode, hf_warning, hf_return
        hf_game_mode = False
        hf_warning = False
        hf_bg()
        renpy.with_statement(effect)
        hf_game_mode = True
        hf_return = len(hf_needed)
        renpy.call_screen("HiddenFolks")
        hf_bg()

    # клик по предмету (перенести его в инвентарь или убрать оттуда)
    def hf_click(item, x, y, h):
        global hf_picked, hf_needed, hf_return
        hf_picked.append(hf_needed.pop(hf_needed.index((item, x, y, h))))
        splay("click")
